...
;;				(define rpath (string-append (get-rpaths file) ":" #$nvidia-libs "/lib"))
				(define rpath (string-join (list (get-rpaths file) (string-append #$nvidia-libs "/lib")) ":"))
				(display (string-append "We're setting rpath to:" rpath "let's seel for 5s just because."))
				(sleep 5)
