    ...
    (arguments '(#:phases
		 (modify-phases %standard-phases
		   ...
		   (add-before 'build 'set-environment-vars
			...
			#t))))
		 #:make-flags '("NVCC_GENCODE='-gencode=arch=compute_60,code=sm_60 -gencode=arch=compute_61,code=sm_61 -gencode=arch=compute_70,code=sm_70 -gencode=arch=compute_80,code=sm_80 -gencode=arch=compute_80,code=compute_80'")
		 #:tests? #f))
